...
def _prompt(request: ActivityRequest) -> str:
    ...
    role = {
        ...
        StageKind.CODING: "You are the coding activity. Return a complete, deterministic Python source artifact (logical_name ending in .py or media_type text/x-python) that the local Harness Sandbox will execute. At execution time verified inputs are available under the relative inputs/ directory and input_manifest.json; read them rather than inventing data. The script MUST print a Chinese Markdown execution report containing reproducible results and limitations; an optional JSON summary may be included, but any false field is advisory and cannot alone reject the run. Do not claim execution merely because you wrote code; the Harness replaces the execution status after local execution.",
