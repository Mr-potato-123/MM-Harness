...
class LocalToolsTest(unittest.TestCase):
    def test_local_catalog_executes_multimodal_adjacent_report_path(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            execution = call("python_execute", {"code": "print(2 + 3)", "timeout_seconds": 10})
            self.assertEqual(execution["stdout"].strip(), "5")
            missing_timeout = call("python_execute", {"code": "print(1)"}, expect_ok=False)
            self.assertEqual(missing_timeout, {})
            blocked = call("python_execute", {"code": "import socket; print('no')", "timeout_seconds": 10}, expect_ok=False)
            self.assertEqual(blocked, {})
            dynamic_import = call("python_execute", {"code": "__import__('socket')", "timeout_seconds": 10}, expect_ok=False)
            self.assertEqual(dynamic_import, {})
            validation = call("validation_run", {"code": "import json; print(json.dumps({'ok': True}))", "timeout_seconds": 10})
