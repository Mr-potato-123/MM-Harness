...
class SolveProblemHarnessTest(unittest.TestCase):
    ...
    def test_each_model_code_review_revision_is_archived_before_next_stage(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            preliminary_report = next(exchange_root.rglob("preliminary_modeling_report.md")).read_text(encoding="utf-8")
            self.assertEqual(preliminary_report.count("Evidence."), 1)
            self.assertNotIn("Model Agent 报告", preliminary_report)
